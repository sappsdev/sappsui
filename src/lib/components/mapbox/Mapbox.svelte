<script lang="ts">
  ...
  onMount(() => {
    ...
    if (markers.length > 0) {
      markers.forEach((marker) => {
        ...
        if (marker.avatar) {
          ...
          if (
            marker.avatar.startsWith("http") ||
            marker.avatar.startsWith("/")
          ) {
            // It's an image URL
        ...
        if (enableRealTimeUpdates) {
          mapMarker.on("dragend", () => {
            const newPosition = mapMarker.getLngLat().toArray() as [
              number,
              number,
            ];
            if (onMarkerUpdate) {
    ...
    map.on("load", () => {
      const mapboxElements = mapContainer.querySelectorAll(
    ...
    map.on("click", async (e) => {
      ...
      try {
        ...
        const locationData = {
          ...
          country: features.context?.find((c: any) =>
            c.id.startsWith("country.")
          )?.text,
          state: features.context?.find((c: any) => c.id.startsWith("region."))
            ?.text,
          city: features.context?.find((c: any) => c.id.startsWith("place."))
            ?.text,
          place: features.text,
  ...
  function updateMarkerPosition(
    ...
    animate = animateMarkerMovement
  ) {
    if (!map) return;

    const markerToUpdate = mapMarkers.find(
      (marker) => (marker as any).markerId === markerId
    );
  ...
  export function updateMarkers(
    ...
    updates.forEach((update) => {
      updateMarkerPosition(
        update.id,
        update.position,
        update.animate !== undefined ? update.animate : animateMarkerMovement
      );
    });
  }
...
<div
  ...
  class={cn("size-full rounded-xl", className)}
></div>
...
<style>
  ...
  :global(.animate-marker-movement) {
    transition: transform var(--animation-duration, 500ms)
